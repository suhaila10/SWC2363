<html>
<head>
<title>delete visitor - admin page llaollao</title>
...
else
{
	...
	if ($_SERVER['REQUEST_METHOD']=='POST')
	{
		...
		{
			//make the query
			$q = "DELETE FROM location WHERE locationID = $id LIMIT 1";
		...
		else
		{
			...
			$q= "SELECT locationName FROM location WHERE locationID =$id";
			$result = @mysqli_query ($connect, $q); //run the query
			...
			if (mysqli_num_rows($result)==1)
			{
				...
				echo'<form action="locationDelete.php" method="post">
